...
static String minuteGraph(WaterHistory& history) {
  const uint8_t bucketMinutes = 5;
  const uint8_t bucketCount = 12;
  uint32_t maxValue = 0;
  uint32_t totalValue = 0;
  ...
  out += formatM3(maxValue);
  out += F(" m3/5 min</strong></span></div><div class=\"minuteBars\" id=\"minuteBars\">");
...
void AppWebServer::sendHtml(const String& body) {
  ...
  html += F(".minutePanel{padding-bottom:12px}.minuteBars{height:160px;display:grid;grid-template-columns:repeat(12,1fr);gap:6px;align-items:end;border-bottom:1px solid #bcccdc;padding-top:8px;overflow:hidden;background:linear-gradient(to top,#f8fafc,#fff)}.minuteWrap{height:100%;display:grid;grid-template-rows:1fr auto;gap:4px;min-width:0;text-align:center;color:#52606d;font-size:9px;font-style:normal}.minuteBar{align-self:end;background:#0b7285;border-radius:3px 3px 0 0;min-height:0}.minuteWrap:nth-child(2n) .minuteBar{background:#147d64}.minuteWrap span{white-space:nowrap;overflow:hidden;text-overflow:clip}");
  ...
  html += F("</main><script>");
  html += F("function byId(i){return document.getElementById(i)}function txt(i,v){const e=byId(i);if(e)e.textContent=v}function pill(i,c){const e=byId(i);if(e)e.className='statusPill '+c}function chip(i,c,t){const e=byId(i);if(e){e.className='chip '+c;e.textContent=t}}function rssiClass(v){if(v===null||v===undefined)return'statusOff';if(v>=-85)return'statusOk';if(v>=-100)return'statusWarn';return'statusAlarm'}function fmt(v){return Number(v||0).toFixed(3)}function refreshMinuteGraph(values){if(!values||!byId('minuteBars'))return;let buckets=new Array(12).fill(0),total=0,max=0;values.forEach((v,idx)=>{v=Number(v)||0;total+=v;const age=59-idx,b=Math.floor(age/5);if(b>=0&&b<12)buckets[b]+=v});buckets.forEach(v=>{if(v>max)max=v});txt('minuteTotal',fmt(total)+' m3');txt('minutePeak',fmt(max)+' m3/5 min');buckets.forEach((v,age)=>{const b=document.querySelector('[data-minute-bar=\"'+age+'\"]');if(!b)return;b.style.height=(!v||!max)?'0%':Math.max(3,Math.round(v*100/max))+'%';b.parentElement.title=(age?'-'+(age*5):'now')+': '+fmt(v)+' m3'})}");
