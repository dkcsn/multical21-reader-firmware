...
void AppWebServer::handleDataJson() {
  ...
  json += F(",\"wifi_ssid\":\"");
  json += WiFi.status() == WL_CONNECTED ? jsonEscape(WiFi.SSID()) : String("");
  json += F("\"");
  json += F(",\"alarms\":{\"burst\":");
...
void AppWebServer::sendHtml(const String& body) {
  ...
  html += F("</span></span>");
  html += F("<span id=\"topWifiPill\" class=\"statusPill ");
  ...
  html += F("\"><span class=\"statusDot\"></span><span id=\"topWifiText\" class=\"statusText\">");
  html += wifiConnected ? htmlEscape(String(cfg.wifiSsid) + " " + String(WiFi.RSSI()) + String(" dBm")) : String("Setup AP");
  html += F("</span></span>");
